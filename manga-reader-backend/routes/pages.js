...
router.post('/', authMiddleware, adminMiddleware, (req, res) => {
  ...
  db.query(query, [req.params.chapterId, page_number, image_url], (err, result) => {
    if (err) {
      return res.status(500).json({ message: 'Lỗi khi thêm trang' });
    }
    res.status(201).json({ id: result.insertId, chapter_id: req.params.chapterId, ...req.body });
  });
});
...
router.put('/:id', authMiddleware, adminMiddleware, (req, res) => {
  ...
  db.query(query, [page_number, image_url, req.params.pageId], (err, result) => {
...
router.delete('/:id', authMiddleware, adminMiddleware, (req, res) => {
  ...
  db.query(query, [req.params.pageId], (err, result) => {
    if (err || result.affectedRows === 0) {
